...
function updateDragons() {
  dragons.forEach((d, i) => {
    ...
    const maxX =
      i === 0 ? canvas.width - d.width - 40 : canvas.width * 0.45 - d.width;
...
function updateFireballs() {
  ...
  fireballs = fireballs.filter((f) => f.x > -100 && f.x < canvas.width + 100);
}
...
function drawUI() {
  ...
  ctx.font = "16px Arial";
  ctx.fillText(
    "← Back | → Forward | SPACE Jump | SPACEx2 Double Jump | X Attack",
    30,
    canvas.height - 40,
  );
...
function draw() {
  ...
  coins.forEach((c) => ctx.drawImage(coinImg, c.x, c.y, 150, 150));
